  ...
  public void step() {
	...
	if (state==MOVETASK) {
		if (detectFront().getObjectType() == IFrontSensorResult.Types.WALL) {    
			if (whoAmI == MARIO) {
				if ((myX > 1400 && myX < 1600) && (myY > 900 && myY < 1100)) {
					sendLogMessage("#MARIO is trying to stay around the center.");
					myMove();
					return;
				} else if (myX > 2800 || myX < 200 || myY > 1800 || myY < 200) {
					state = TURNLEFTTASK;
