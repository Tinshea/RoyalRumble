...
;
  ...
  public void step() {
	...
	boolean allyOnLeft = false;
	boolean allyOnRight = false;
	...
	for (IRadarResult o: detectRadar()) {
		if (o.getObjectType() == Types.TeamMainBot || o.getObjectType() == Types.TeamSecondaryBot) {
			System.out.println("Detected object: " + o.getObjectType() + ", Direction: " + o.getObjectDirection() + ", Distance: " + o.getObjectDistance());
			...
			if (relativeDirection > Math.PI/2 && relativeDirection < 3*Math.PI/2) {
				allyOnLeft = true;
			}
			
			// Right side detection - between 270° and 360° OR between 0° and 90° relative to heading
			if (relativeDirection > 3*Math.PI/2 || relativeDirection < Math.PI/2) {
				allyOnRight = true;
			}
		}
	}
