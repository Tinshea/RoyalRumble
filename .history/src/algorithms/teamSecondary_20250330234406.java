...
;
  ...
  public void step() {
    ...
    if (state==FLEE) {
        // Check for allies in radar before deciding escape direction
        boolean alliesOnRight = false;
        boolean alliesOnLeft = false;
        double rightDirection = normalizeRadian(getHeading() + Math.PI/2);
        double leftDirection = normalizeRadian(getHeading() - Math.PI/2);
        
        for (IRadarResult o: detectRadar()) {
            if (o.getObjectType() == Types.TeamMainBot || o.getObjectType() == Types.TeamSecondaryBot) {
                double allyDirection = normalizeRadian(o.getObjectDirection());
                // Check if ally is roughly to the right
                if (Math.abs(normalizeRadian(allyDirection - rightDirection)) < Math.PI/3) {
                    alliesOnRight = true;
                }
                // Check if ally is roughly to the left
                if (Math.abs(normalizeRadian(allyDirection - leftDirection)) < Math.PI/3) {
                    alliesOnLeft = true;
                }
            }
        }
        
        // Determine turning direction to avoid allies
        Direction turnDirection;
        if (alliesOnRight && !alliesOnLeft) {
            // Ally on right, turn left
            turnDirection = Direction.LEFT;
        } else if (alliesOnLeft && !alliesOnRight) {
            // Ally on left, turn right
            turnDirection = Direction.RIGHT;
        } else {
            // Either allies on both sides or no allies, use original logic or pick random
            turnDirection = Math.random() < 0.5 ? Direction.RIGHT : Direction.LEFT;
        }
